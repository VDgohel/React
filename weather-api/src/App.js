...
function App() {
  ...
  const search = () => {
    axios.get(`http://api.weatherapi.com/v1/current.json?key=9f2151721c114399a8d61716253007&q=${location}'&aqi=no`).then((response) => (
      ...
      {locationdata && (
        <>
          ...
          <div className="card weather-icon">
            ...
            <div className="temp">{locationdata.current.dewpoint_c}</div>
          ...
          <div className="card">
            ...
            <div className="data-pair">
              <div className="data-label">Gust Speed</div>
              <div className="data-value">{locationdata.current.gust_kph} kph</div>
            </div>
          </div>
